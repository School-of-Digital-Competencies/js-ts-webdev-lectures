import { FormConfig, FormField, FormValues, Icon, InputField, SelectField, TextareaField, CheckboxField, RadioField } from "./components/types";
...
import { createValidationSchema, showValidationErrors, clearValidationErrors } from "./components/validation";
...
export function createForm(config: FormConfig): HTMLElement {
    ...
    if (config.submitButton || config.cancelButton) {
        ...
        if (config.submitButton) {
            ...
            submitButton.className = `button is-${config.submitButton.color || 'primary'}`;
        ...
        if (config.cancelButton) {
            ...
            cancelButton.className = `button is-${config.cancelButton.color || 'light'}`;
